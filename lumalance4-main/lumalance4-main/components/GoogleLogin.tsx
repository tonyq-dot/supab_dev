...
export default function GoogleLogin({ 
  ...
  const handleGoogleSuccess = useCallback(async (response: any) => {
    try {
      ...
      const { data, error } = await api.google.authCallback({
      ...
      if (data?.token && data?.refreshToken) {
        // Store tokens using API client functions
        setToken(data.token);
...
// Alternative button component for custom styling
export function GoogleLoginButton({ 
  ...
  const handleGoogleClick = useCallback(async () => {
    try {
      ...
      const { data, error } = await api.google.getAuthUrl();
